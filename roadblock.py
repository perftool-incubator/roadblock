...
def message_handle (message):
    ...
    if msg_uuid in t_global.processed_messages:
        if t_global.args.debug:
            print("DEBUG: I have already processed this message! [%s]" % (msg_uuid))
        return(0)
    else:
        if t_global.args.debug:
            print("DEBUG: adding uuid='%s' to the processed messages list" % (msg_uuid))
        t_global.processed_messages[msg_uuid] = True

    msg_command = message_get_command(message)

    if msg_command == "timeout-ts":
        print("Received an updated timeout from the roadblock")
        ...
        if t_global.args.roadblock_role == 'leader':
            ...
            if msg_sender in t_global.followers['online']:
                print("Received online message from '%s'" % (msg_sender))
                del t_global.followers['online'][msg_sender]
            elif msg_sender in t_global.args.roadblock_followers:
                print("Did I already process this online message from follower '%s'?" % (msg_sender))
            else:
                print("Received online message from unknown follower '%s'" % (msg_sender))
        ...
        if t_global.initiator:
            print("Initiator received 'all-online'")
            t_global.mirror_busB = False
        else:
        ...
        if t_global.args.roadblock_role == 'leader':
            if t_global.args.debug:
                print("DEBUG: leader got a 'follower-ready'")
            ...
            if msg_sender in t_global.followers['ready']:
                print("Received %s message from '%s'" % (msg_type, msg_sender))
                del t_global.followers['ready'][msg_sender]
            elif msg_sender in t_global.args.roadblock_followers:
                print("Did I already process this %s message from follower '%s'?" % (msg_type, msg_sender))
            else:
                print("Received %s message from unknown follower '%s'" % (msg_type, msg_sender))
        ...
        if t_global.args.roadblock_role == 'leader':
            if t_global.args.debug:
                print("DEBUG: leader got a 'follower-gone'")
        ...
        print("I just learned that the initiator has type='%s' and id='%s'" % (t_global.initiator_type, t_global.initiator_id))
...
def cleanup():
    ...
    if t_global.args.debug:
        print("DEBUG: Processed Messages:")
...
def do_timeout():
    ...
    cleanup()
    print("Roadblock failed with timeout")
    exit(-3)
...
def main():
    ...
    if t_global.initiator:
        ...
        for msg in t_global.pubsubcon.listen():
            ...
            if t_global.args.debug:
                print("DEBUG: initiator received msg=[%s] on busB" % (msg_str))
            ...
            if not message_validate(msg):
                ...
                if not message_for_me(msg):
                    if t_global.args.debug:
                        print("DEBUG: initiator received a message which is not for me! [%s]" % (msg_str))
                else:
                    if t_global.args.debug:
                        print("DEBUG: initiator received a message for me! [%s]" % (msg_str))
            ...
            if not t_global.mirror_busB:
                if t_global.args.debug:
                    print("DEBUG: initiator stopping busB mirroring to busA")
                break
    else:
        msg_list_index = -1
        get_out = False
        while t_global.watch_busA:
            ...
            if len(msg_list):
                for msg_str in msg_list:
                    msg_list_index += 1
                    if t_global.args.debug:
                        print("DEBUG: received msg=[%s] on busA with status_index=[%d]" % (msg_str, msg_list_index))
                    ...
                    if not message_validate(msg):
                        ...
                        if not message_for_me(msg):
                            if t_global.args.debug:
                                print("DEBUG: received a message which is not for me! [%s]" % (msg_str))
                        else:
                            if t_global.args.debug:
                                print("DEBUG: received a message which is for me! [%s]" % (msg_str))
                            ret_val = message_handle(msg)
                            if ret_val:
                                return(ret_val)
    ...
    if t_global.args.debug:
        print("DEBUG: moving to common busB watch loop")

    while t_global.watch_busB:
        for msg in t_global.pubsubcon.listen():
            msg_str = msg['data'].decode()
            if t_global.args.debug:
                print("DEBUG: received msg=[%s] on busB" % (msg_str))
            ...
            if not message_validate(msg):
                ...
                if not message_for_me(msg):
                    ...
                    if t_global.args.debug:
                        print("DEBUG: received a message for me! [%s]" % (msg_str))
                    ret_val = message_handle(msg)
                    if ret_val:
                        return(ret_val)
